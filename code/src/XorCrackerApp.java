...
public class XorCrackerApp {
    ...
    private static void run() throws IOException {
        try {
            String values = "_- abcdefg";
            System.out.println("Number of candidate keys: " + values.length());
            for (char mostFrequent : values.toCharArray()) {
                int[] key = getCandidateKey(filename, keylength, (int) mostFrequent);
                System.out.println(mostFrequent + ": " + HexTools.intArrayToHexString(key));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static int[] getCandidateKey(String filename, int keylength, int mostFrequentValue) throws IOException {
        ...
        for (int keyByte = 0; keyByte < keylength; keyByte++) {
            ...
            //DONE: Determine byte 'keyByte' of the key
